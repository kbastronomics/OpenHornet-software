...
#define COMM1_CLK_3_3V  39
#define COMM2_CLK_3_3V  10
#define OP_DIGITS       4
 ...
 * 
 */
typedef struct dcs_meta_data 
{
u_int8_t    MissionStart  =  false;
...
#define COM_DIGITS      4
#define COMM_DIGITS     1
#define OP_BRIGHTNESS   7
#define COMM_BRIGHTNESS 7
...
TM1640Anode GN1640[6] {
  TM1640Anode(DIO_3_3V, OP1_CLK_3_3V, OP_DIGITS, true, OP_BRIGHTNESS), //OP Display 1; data, clock, digit count, active, brightness
  TM1640Anode(DIO_3_3V, OP2_CLK_3_3V, OP_DIGITS, true, OP_BRIGHTNESS), //OP Display 2; data, clock, digit count, active, brightness
  TM1640Anode(DIO_3_3V, OP3_CLK_3_3V, OP_DIGITS, true, OP_BRIGHTNESS), //OP Display 3; data, clock, digit count, active, brightness
  TM1640Anode(DIO_3_3V, OP4_CLK_3_3V, OP_DIGITS, true, OP_BRIGHTNESS), //OP Display 4; data, clock, digit count, active, brightness
  TM1640Anode(DIO_3_3V, OP5_CLK_3_3V, OP_DIGITS, true, OP_BRIGHTNESS), //OP Display 5; data, clock, digit count, active, brightness
  TM1640Anode(DIO_3_3V, COMM1_CLK_3_3V, COM_DIGITS, true, COMM_BRIGHTNESS) //CUEs		 ; data, clock, digit count, active, brightness
};
 ...
 * @brief
 */
TM1640Anode COMM1(DIO_3_3V, COMM1_CLK_3_3V, COMM_DIGITS, true, COMM_BRIGHTNESS); // DA=4, CLK=7
TM1640Anode COMM2(DIO_3_3V, COMM2_CLK_3_3V, COMM_DIGITS, true, COMM_BRIGHTNESS); // DA=4, CLK=7
...
TM1640 GN1640_COMM[2] {
  TM1640(DIO_3_3V, COMM1_CLK_3_3V),
  TM1640(DIO_3_3V, COMM2_CLK_3_3V)
};

/**
 * @breif handle comm displays
 */
...
/**
 * handle cue displays for OP4-5 thru COMM2
 */
...
//------------ CUE SYMBOL --------------------
TM1640Anode CUE123(DIO_3_3V, CUE_CLK_3_3V, 8, true, OP_BRIGHTNESS);


//-------------- UFC Brightness ---------------
uint16_t UCF_brightness = 7;
 ...
 * @breif 
 */
void sendAsciiChar_OH(uint16_t op_display, byte pos, char c) {
 ...
 * @breif 
 */
void update_op_display(uint16_t disp_num) {
  uint16_t disp_pos = 0;

  for (uint16_t digit_num = 0; digit_num < OP_DIGITS; digit_num++) {
 ...
 * @breif 
 */
void display_comm_char(char * comm_char, uint16_t n_display) {
  uint16_t array_pos;
 ...
 * @breif 
 */
void update_op_cue(uint16_t disp_num) {
  switch (disp_num) {
...
typedef DcsBios::PotentiometerEWMA < POLL_EVERY_TIME, 192, 15 > OHPotentiometer;

OHPotentiometer ufcBrt("UFC_BRT", A2, false, 0, 8192);
OHPotentiometer ufccom1vol("UFC_COMM1_VOL", A0, true, 0, 8192);
OHPotentiometer ufccom2vol("UFC_COMM2_VOL", A1, false, 0, 8192);
...
void bootMessage(bool noPause, bool noClear, bool noCue) {
  ...
  for (uint16_t display_num = 0; display_num < 5; display_num++) {
...
/**
 * @brief Breath Effect when no connected.
 */
...
void breath(void) {
  ...
  for (uint16_t i = BACKLIGHT_LEDS+1; i < (BACKLIGHT_LEDS + 4) ; i++) {
    
    ws2812b[i] = CHSV(hue, sat, val);
  }
  FastLED.show();
}
...
void setup() {
  ...
  UFCkeypad.pinMode(UFC_IPSW, INPUT); //5
  UFCkeypad.pinMode(UFC_BITSW, INPUT); //12
  UFCkeypad.pinMode(UFC_EMCON, INPUT); //17
...
void loop() {
  ...
  // //     UFCkeypad.digitalWrite(UFC_GO, gostate);   
  // //     last_go = millis(); 
  // //     gostate = !gostate;
  // } else {
  ...
  if (DCSMetaData.MissionStart == DSCSTOPPED ) {
    UFCkeypad.digitalWrite(UFC_GO, LOW); 
