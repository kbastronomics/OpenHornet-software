...
//#define DCSBIOS_USE_MATRIX_SWITCHES
...
typedef DcsBios::RotaryEncoderT<POLL_EVERY_TIME, DcsBios::TWO_STEPS_PER_DETENT> RotaryEncoder2step;

RotaryEncoder2step ufcComm1ChannelSelect("UFC_COMM1_CHANNEL_SELECT", "DEC", "INC", ENC1A, ENC1B);
DcsBios::Switch2Pos ufcComm1Pull("UFC_COMM1_PULL", ENC1PB);

RotaryEncoder2step  ufcComm2ChannelSelect("UFC_COMM2_CHANNEL_SELECT", "DEC", "INC", ENC2A, ENC2B);
...
const byte apins[] = { 1, 2, 3 };
const int aio_count = sizeof(apins)/sizeof(apins[0]);
long last_analog = 0;
...
void display_comm_char(char* comm_char,int n_display){
    ...
    switch (comm_char[1]){
      case '1': array_pos = 1; break;  // 1
      case '2': array_pos = 2; break;  // 2
      case '3': array_pos = 3; break;  // 3
      case '4': array_pos = 4; break;  // 4
      case '5': array_pos = 5; break;  // 5
      case '6': array_pos = 6; break;  // 6
      case '7': array_pos = 7; break;  // 7
      case '8': array_pos = 8; break;  // 8
      case '9': array_pos = 9; break;  // 9
      case 'M': array_pos = 21; break; // M
      case 'G': array_pos = 22; break; // G
      case 'C': array_pos = 23; break; // C
      case 'S': array_pos = 24; break; // S   {0x02, 0x0B, 0x02, 0x0B}, //S
      case ' ': array_pos = 0; break;  // BLANK
    }
...
/**
 * @BREIF DCS-BIOS INTERFACE SETTINGS
 */
...
void UFC_OPTION_DISPLAY_1Change(char* newValue){
  strcpy(op_display[OP1].content,newValue);
...
void UFC_OPTION_DISPLAY_2Change(char* newValue){
  ...
  // }
  strcpy(op_display[OP2].content,newValue);
...
void UFC_COMM_DISPLAY_1Change(char* newValue){
  Serial.printf("UFC_COMM_DISPLAY_1Change(%s)\n",newValue);
...
void UFC_COMM_DISPLAY_2Change(char* newValue) {
  Serial.printf("UFC_COMM_DISPLAY_2Change(%s)\n",newValue);
...
/**
 * @todo Get POTS workign correctly
 */
...
//DcsBios::Potentiometer ufcBrt("UFC_BRT", 3,false,0,8192);
//DcsBios::Potentiometer ufccom1vol("UFC_COMM1_VOL", 1,true,0,8192);
//DcsBios::Potentiometer ufccom2vol("UFC_COMM2_VOL", 2,false,0,8192);
...
//-------------- APKEYBOARD -----------------
//DcsBios::ApSwitch2Pos ufcAp("UFC_AP", ED13);
...
// --------------- show welcome -----------------
void showWelcome(bool noPause,bool noClear,bool noCue) {
  char newValue[2];
...
void setup() {
  ...
  if (!UFCkeypad.begin(TCA8418_DEFAULT_ADDR, &Wire)) {
      ...
      Serial.println("UFCkeypad found at 0x34");
    }
    ...
    //Serial.printf("UFCkeypad.pinMode(NOGOLED, OUTPUT)\n");
    UFCkeypad.pinMode(NOGOLED, OUTPUT); // NOGO LED
    //Serial.printf("UFCkeypad.pinMode(GOLED, OUTPUT)\n");
    ...
    // Serial.printf("pinMode()\n");
    // for (int i=0; i < 2; i++){
    //   pinMode(encoderCLK[i], INPUT_PULLUP);
    //   pinMode(encoderDATA[i], INPUT_PULLUP);
    // }

    // Serial.printf("attachInterrupt()\n");
    // attachInterrupt(digitalPinToInterrupt(encoderCLK[0]), handle_encoder1, CHANGE);
    // attachInterrupt(digitalPinToInterrupt(encoderDATA[0]), handle_encoder1, CHANGE);
    // attachInterrupt(digitalPinToInterrupt(encoderCLK[1]), handle_encoder2, CHANGE);
    // attachInterrupt(digitalPinToInterrupt(encoderDATA[1]), handle_encoder2, CHANGE);
    ...
    Serial.printf("Show UFC Welcome Message\n");
    showWelcome(true,true,true);;
...
void loop() {
  ...
  // for (int i = 0; i < 2 ; i++){
  //   if (encoder_update[i]){
  //     Serial.print("Encoder ");Serial.print(i + 1);Serial.print(" ");Serial.println(encoder_count[i]);
  //     encoder_update[i] = false;
  //   }
  // }
  
  //----------- Analog  ----------  
  // if ((millis() - last_analog) > 2000){
  //   for (int i=0; i < aio_count; i++){
  //     Serial.print("Analog ");Serial.print(i);Serial.print(" ");Serial.println(analogRead(apins[i]));
  //   }
  //   last_analog = millis();
  // }
